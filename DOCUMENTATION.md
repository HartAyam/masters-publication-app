...
| **Rollup failed to resolve import "jspdf" (Local Build Fail)** | Dependencies in `package.json` (such as `jspdf`) are not installed in the local `node_modules` directory, or there is a stale cache/conflict in the local folder structure. | Run `npm install` (or a clean `npm ci`) on your local machine to populate the `node_modules` folder. If the error persists, clear the Vite cache with `npm run clean && rm -rf node_modules package-lock.json && npm install`. |
| **HTTP Error: 400, Can't release to channels/live (Github Action Fail)** | This occurs when using the legacy static `action-hosting-deploy` action in your GitHub repo alongside **Firebase App Hosting**. Since App Hosting manages the full-stack server deployment, deploying static files with the legacy action is redundant. It also fails when there are no file changes since the last build, as the Hosting API rejects duplicate releases. | **Completely remove the legacy GitHub Action workflow files** (e.g. `.github/workflows/firebase-hosting-merge.yml` or `firebase-hosting-pull-request.yml`) from your GitHub repository. Since you are using **Firebase App Hosting** (Phase 4), your CI/CD is fully managed automatically upon pushing to your tracking branch. You do not need the old static hosting deploy action. |
