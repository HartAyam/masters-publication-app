...
async function run() {
  ...
  txs.forEach(d => {
    ...
    console.log({
      ...
      items: data.items?.map((i: any) => `${i.productName} (${i.quantity})`)
    });
  });
  process.exit(0);
}
